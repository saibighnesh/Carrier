...
t("decodes from an arbitrary subset of surviving parity rows", () => {
  const d = mk(10,30), p = rsEncode(d,4);
...
t("refuses rather than guessing when redundancy is short", () => {
  const d = mk(8,20), p = rsEncode(d,1);
  const pres = new Map(d.map((s,i)=>[i,s])); [1,4].forEach(i=>pres.delete(i));
  assert.equal(rsDecode(8,20,pres,new Map([[0,p[0]]])), null);
});
t("parity is itself Base64 text — the no-inflation property", () => {
  const d = mk(24,50,41), p = rsEncode(d,6);
...
t("exhaustive: 32 data + 4 parity, every 4-loss pattern rebuilds exactly", () => {
  const n=32, len=12, d = mk(n,len,9), p = rsEncode(d,4);
  let c = 0;
...
t("symbol <-> Base64 round trip is exact, and pads short shards with zero", () => {
...
t("redundancy levels are sane and never round down to zero", () => {
